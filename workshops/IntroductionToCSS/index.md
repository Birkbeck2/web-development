...
4)	Internal CSS. First let’s put some CSS in a ```<style>``` tag inside the head. This is called internal CSS:
...
5)	Inline CSS. If you want to only style one part of a paragraph instead of all of it, use the ```<span>``` tag.
